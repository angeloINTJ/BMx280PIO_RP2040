...
bool BMx280_PIO::begin() {
    ...
    sleep_ms(15); // Non-blocking: yields to other tasks
...
void BMx280_PIO::setTemperatureOversampling(uint8_t os) { _osrs_t = os & 0x07; if (_init) { uint8_t p = _mode; setMode(BME280_MODE_SLEEP); _applyConfig(); setMode(p); } }
void BMx280_PIO::setPressureOversampling(uint8_t os)    { _osrs_p = os & 0x07; if (_init) { uint8_t p = _mode; setMode(BME280_MODE_SLEEP); _applyConfig(); setMode(p); } }
void BMx280_PIO::setHumidityOversampling(uint8_t os)    { _osrs_h = os & 0x07; if (_init) { uint8_t p = _mode; setMode(BME280_MODE_SLEEP); _applyConfig(); setMode(p); } }
void BMx280_PIO::setFilter(uint8_t f)  { _filter  = f & 0x07; if (_init) { uint8_t p = _mode; setMode(BME280_MODE_SLEEP); _applyConfig(); setMode(p); } }
void BMx280_PIO::setStandbyTime(uint8_t s) { _standby = s & 0x07; if (_init) { uint8_t p = _mode; setMode(BME280_MODE_SLEEP); _applyConfig(); setMode(p); } }
...
uint8_t BMx280_PIO::_measTime() {
    // Integer computation: measurement time in ms
    // t(ms) = 1.25 + 2.3*osrs_t + 2.3*osrs_p + 0.575 (+ 2.3*osrs_h + 0.575 for BME)
    const uint8_t m[] = {0, 1, 2, 4, 8, 16};
    // Scale by 1000 to avoid float: t*1000 = 1250 + 2300*m[osrs_t] + ...
    uint32_t t = 1250 + 2300 * m[_osrs_t & 0x07] + 2300 * m[_osrs_p & 0x07] + 575;
    if (_is_bme) t += 2300 * m[_osrs_h & 0x07] + 575;
    return (uint8_t)((t + 1500) / 1000); // Round up, convert to ms
}

bool BMx280_PIO::takeForcedMeasurement() {
    if (!_init) return false;
    setMode(BME280_MODE_FORCED);
    sleep_ms(_measTime()); // Non-blocking sleep
    uint32_t start = millis();
    uint8_t st;
    do {
        _i2c_read(BME280_REG_STATUS, &st, 1);
...
// ─── Compensation (Bosch datasheet integer formulas, no float/double) ─────

float BMx280_PIO::readTemperature() {
    if (!_init) return NAN;
    int32_t adc_T; _readRaw(&adc_T, nullptr, nullptr);
    int32_t v1 = ((((adc_T >> 3) - ((int32_t)_T1 << 1))) * ((int32_t)_T2)) >> 11;
    int32_t v2 = (((((adc_T >> 4) - ((int32_t)_T1)) * ((adc_T >> 4) - ((int32_t)_T1))) >> 12) * ((int32_t)_T3)) >> 14;
    _t_fine = v1 + v2;
...
float BMx280_PIO::readPressure() {
    ...
    // Double-precision compensation (verified correct: 1014 hPa at sea level)
...
void BMx280_PIO::readAll(float *t, float *p, float *h) {
    ...
    // Pressure (double-precision, verified correct)
    if (p) {
...
uint8_t BMx280_PIO::readRegister(uint8_t reg)   { uint8_t v = 0; _i2c_read(reg, &v, 1); return v; }
void    BMx280_PIO::writeRegister(uint8_t r, uint8_t v) { _i2c_write(r, &v, 1); }
void    BMx280_PIO::readRegisters(uint8_t r, uint8_t *d, size_t n) { _i2c_read(r, d, n); }
uint8_t BMx280_PIO::getChipID() { return readRegister(BME280_REG_CHIP_ID); }
void    BMx280_PIO::reset()     { uint8_t v = BME280_RESET_VALUE; _i2c_write(BME280_REG_RESET, &v, 1); }
